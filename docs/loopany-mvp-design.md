...
- **Verification**: `pnpm -r typecheck` green (both packages); 58 server tests green incl. 7 new gateway tests (cross-team success incl. existing-machine reuse, fail-closed minter mismatch, fail-closed non-member, superadmin authorization, no-claim back-compat, same-team/open-mode no-check, membership-scoped listing).
