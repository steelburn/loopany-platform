...
#   LOOPANY_TOKEN=<tok> LOOPANY_SERVER_URL=http://127.0.0.1:8787 \
...
export LOOPANY_DATA_DIR="${LOOPANY_DATA_DIR:-$HOME/.loopany}"

pids=()
cleanup() { for p in "${pids[@]}"; do kill "$p" 2>/dev/null || true; done; }
trap cleanup EXIT INT TERM

echo "▶ backend (main.ts) on :$BACKEND_PORT  ·  data dir $LOOPANY_DATA_DIR"
( cd "$SERVER_DIR" && LOOPANY_PORT="$BACKEND_PORT" pnpm exec tsx src/main.ts ) &
pids+=($!)

echo "▶ UI (vite dev) on :$UI_PORT  ·  proxying http://127.0.0.1:$BACKEND_PORT"
( cd "$SERVER_DIR" && LOOPANY_PORT="$UI_PORT" LOOPANY_API_BASE="http://127.0.0.1:$BACKEND_PORT" pnpm dev ) &
pids+=($!)
