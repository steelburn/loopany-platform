...
- **`loopany log`** — the quick survey. Prints the recent runs newest-first, concise and scannable: each run's outcome, its metrics, and its `session` id — straight from the server. Start here to see how runs have been going and to spot patterns across them without hunting for files. Add `--transcript` for an inline clipped peek at each run's transcript when you want more than the summary.
- **The session JSONL** — the deep dive. Each run wrote a claude-code `<session>.jsonl` on this machine. Take the run's `session` id from `loopany log` and `find ~/.claude/projects -name '<session>.jsonl'`, then read it — the full, unclipped record of what the agent actually did: metrics computed but never reported, recurring friction, wasted effort, and the SAME mechanical steps repeated every run. Even `--transcript` is clipped, so when a decision hinges on exactly what happened in a run, read its session (tail or grep it — don't slurp a multi-MB file).

Which lens you need depends on the lever. For the **task** and **workflow** levers the sessions are the primary input — a sharper Spec or a liftable deterministic step only shows in what the agent actually did, so survey with `loopany log`, then read the sessions before concluding there is nothing to change. For **schema / UI** the reported metrics usually tell the story; drop into a session only when a call hinges on behavior the data does not explain.
