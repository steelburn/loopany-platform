...
- **`loopany log`** — the quick survey. Prints the recent runs newest-first: each run's outcome, its metrics, a clipped transcript, and its `session` id — straight from the server. Start here to see how runs have been going and to spot patterns across them without hunting for files.
- **The session JSONL** — the deep dive. Each run wrote a claude-code `<session>.jsonl` on this machine. Take the run's `session` id from `loopany log` and `find ~/.claude/projects -name '<session>.jsonl'`, then read it — the full, unclipped record of what the agent actually did: metrics computed but never reported, recurring friction, wasted effort, and the SAME mechanical steps repeated every run. `loopany log` is clipped, so when a decision hinges on exactly what happened in a run, read its session (tail or grep it — don't slurp a multi-MB file).
