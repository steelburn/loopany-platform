...
Act only through the `loopany` command on your PATH. The verbs below all act on THIS
loop — no id needed. (Owner-side authoring uses a different surface, `loopany edit
<id> --json`, which you do not have — you have run-token verbs only.) `loopany help`
lists them; for the full syntax and contract of any verb, use the loopany skill
...
- **Dashboard UI / metric schema / workflow** — only if the requested change calls for
  ...
  `<script>`/handlers/`<svg>`), `loopany set-schema --file <path>` (a JSON array of
  `{key, label?, unit?}`, additive), `loopany set-workflow --file <path>` (the
