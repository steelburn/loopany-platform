...
desc: "Long-horizon self-driving task — advance one step per tick from task.md, self-pace, run to convergence"
tags: [autopilot, agentic, self-pacing, case2]
slots:
  - {name: taskFile, prompt: "Path to the task doc (task.md) — its Spec and running progress both live there", required: true, kind: path}
